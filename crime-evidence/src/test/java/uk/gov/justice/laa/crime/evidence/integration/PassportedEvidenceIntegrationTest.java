...
class PassportedEvidenceIntegrationTest extends IntegrationTestBase {
    ...
    private static final String MAAT_API_PASSPORTED_EVIDENCE_URL = "/api/internal/v1/assessment/passported/evidence";
    ...
    void givenValidRequest_whenGetEndpointIsCalled_thenPassportEvidenceResponseReturned() throws Exception{
        ...
        wiremock.stubFor(get(urlEqualTo(MAAT_API_PASSPORTED_EVIDENCE_URL + "/" + PASSPORTED_ASSESSMENT_ID))
            .willReturn(WireMock.ok()
                .withHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
                .withBody(response)));

        MvcResult result = mvc.perform(RequestBuilderUtils.buildRequest(HttpMethod.GET, ENDPOINT_URL + "/" + PASSPORTED_ASSESSMENT_ID))
            .andReturn();
    ...
    void givenUnauthorisedRequest_whenGetEndpointIsCalled_thenUnauthorisedErrorResponseReturned() throws Exception {
        mvc.perform(RequestBuilderUtils.buildRequest(HttpMethod.GET, ENDPOINT_URL + "/" + PASSPORTED_ASSESSMENT_ID, false))
            .andExpect(status().isUnauthorized());
    }

    @Test
    void givenErrorResponseFromMaatApi_whenGetEndpointIsCalled_thenInternalServerErrorResponseReturned() throws Exception {
        wiremock.stubFor(get(urlEqualTo(MAAT_API_PASSPORTED_EVIDENCE_URL + "/" + PASSPORTED_ASSESSMENT_ID))
